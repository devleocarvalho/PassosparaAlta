document.addEventListener('DOMContentLoaded', () => {
    const perguntas = [
        {
            texto: "O paciente está em coma profundo, com rigidez de descerebração ou aspecto moribundo?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "V",
                    mortalidade: "70 - 100%",
                    descricao: "Coma profundo, rigidez de descerebração ou aspecto moribundo."
                },
                { texto: "Não", proxima: 1 }
            ]
        },
        {
            texto: "O paciente apresenta sopor moderado a profundo ou hemiparesia moderada a severa?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "IV",
                    mortalidade: "60 - 70%",
                    descricao: "Sopor moderado a profundo ou hemiparesia moderada a severa."
                },
                { texto: "Não", proxima: 2 }
            ]
        },
        {
            texto: "O paciente está letárgico, confuso ou com déficit focal moderado?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "III",
                    mortalidade: "10 - 15%",
                    descricao: "Sopor leve, letargia, confusão ou déficit focal moderado."
                },
                { texto: "Não", proxima: 3 }
            ]
        },
        {
            texto: "O paciente apresenta cefaleia moderada a severa e rigidez de nuca com possível comprometimento de nervos cranianos?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "II",
                    mortalidade: "5 - 10%",
                    descricao: "Cefaleia moderada a severa, rigidez de nuca com comprometimento de nervos cranianos."
                },
                { texto: "Não", proxima: 4 }
            ]
        },
        {
            texto: "O paciente está consciente, estável, mas com déficit neurológico estabelecido?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "Ia",
                    mortalidade: "2 - 5%",
                    descricao: "Sem comprometimento de consciência, estável, mas com déficit neurológico estabelecido."
                },
                { texto: "Não", proxima: 5 }
            ]
        },
        {
            texto: "O paciente está assintomático, apenas com cefaleia leve ou rigidez de nuca?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "I",
                    mortalidade: "1 - 2%",
                    descricao: "Assintomático, cefaleia leve ou rigidez de nuca."
                },
                { texto: "Não", proxima: 6 }
            ]
        },
        {
            texto: "O aneurisma foi diagnosticado incidentalmente, sem sintomas e ainda não roto?",
            opcoes: [
                {
                    texto: "Sim",
                    grau: "0",
                    mortalidade: "0%",
                    descricao: "Aneurisma assintomático não roto, diagnosticado incidentalmente."
                },
                { // Última opção "Não" leva ao resultado "Indeterminado"
                    texto: "Não",
                    grau: "Indeterminado",
                    mortalidade: "Reavalie os dados clínicos.",
                    descricao: "Não foi possível classificar com os dados fornecidos."
                }
            ]
        }
    ];

    let etapa = 0;

    // Elementos do DOM
    const questionContainer = document.getElementById("question-container");
    const divPergunta = document.getElementById("pergunta");
    const divBotoes = document.getElementById("botoes");
    const divResultado = document.getElementById("resultado");
    const resultGrauEl = document.getElementById("result-grau");
    const resultQuadroClinicoEl = document.getElementById("result-quadro-clinico");
    const resultMortalidadeEl = document.getElementById("result-mortalidade");
    const restartBtn = document.getElementById("restartBtn"); // Botão de recomeçar

    function mostrarPergunta() {
        // Esconde o resultado e mostra o container da pergunta
        questionContainer.classList.remove("hidden");
        divResultado.classList.add("hidden");
        restartBtn.classList.add("hidden"); // Esconde o botão de recomeçar durante o quiz

        const perguntaAtual = perguntas[etapa];
        divPergunta.innerText = perguntaAtual.texto;
        divBotoes.innerHTML = ""; // Limpa os botões anteriores

        perguntaAtual.opcoes.forEach(opcao => {
            const botao = document.createElement("button");
            botao.innerText = opcao.texto;
            botao.onclick = () => {
                if (opcao.grau !== undefined) {
                    // Se a opção tem um grau definido, significa que é um resultado final
                    mostrarResultado(opcao.grau, opcao.mortalidade, opcao.descricao);
                } else {
                    // Se não tem grau, significa que avança para a próxima pergunta
                    etapa = opcao.proxima;
                    mostrarPergunta();
                }
            };
            divBotoes.appendChild(botao);
        });
    }

    function mostrarResultado(grau, mortalidade, descricao) {
        // Esconde o quiz e mostra o resultado
        questionContainer.classList.add("hidden");
        divResultado.classList.remove("hidden"); // Mostra a div de resultado
        divResultado.classList.add("fadeIn"); // Aplica a animação

        // Preenche os elementos de resultado
        resultGrauEl.innerHTML = `<strong>Grau:</strong> ${grau}`;
        resultQuadroClinicoEl.innerHTML = `<strong>Quadro clínico:</strong> ${descricao}`;
        resultMortalidadeEl.innerHTML = `<strong>Mortalidade perioperatória:</strong> ${mortalidade}`;

        restartBtn.classList.remove("hidden"); // Mostra o botão de recomeçar no final
    }

    // Event listener para o botão de recomeçar (se existir)
    restartBtn.addEventListener('click', () => {
        etapa = 0; // Reseta a etapa para a primeira pergunta
        mostrarPergunta(); // Inicia o quiz novamente
    });

    // Inicializar o quiz ao carregar a página
    window.onload = mostrarPergunta;
});